            ...
            @foreach ($products as $product)
                <div class="card card-flush flex-row-fluid p-6 pb-5 mw-100 cursor-pointer add-product" data-item="{{ $product }}">
                    <div class="card-body text-center">
                        <img src="{{ url('./assets/media/uploads/products/'.$product->image) }}" alt="{{ $product->name }}" class="rounded-3 mb-4 w-150px h-150px w-xxl-200px h-xxl-200px">
                        <div class="mb-2">
                        ...
                        <span class="text-success text-end fw-bold fs-1">IDR {{ $product->price }}</span>
                    </div>
                </div>
            @endforeach
            </div>
...
<script>
    $(document).ready(function(){
        $listOrder = $("#card-order .list-order");
